...
Public Class ucFavManageVer2
    ...
    Private _addDt As New DataTable

    Public WriteOnly Property addDt As DataTable
        Set(value As DataTable)
            _addDt.Reset()
            _addDt = Nothing
            _addDt = New DataTable
            _addDt = value
            SettingAddStockListDataTable()
        End Set
    End Property

    Private _addDs As New DataSet
    ...
    Private Sub SettingAddStockListDataTable()
        Dim row As Integer = 0

        dgvAddStock.Rows.Clear()

        For Each dr As DataRow In _addDt.Rows
...
#Region " DeleteRecordPsi01 "
    Private Function DeleteRecordPsi01(ByVal stockId As String, ByVal interId As Integer) As Boolean
        ...
        Try
            ...
            conn.ExecuteNonQuery("p_psi01Del", arrParam)
