...
#End Region
    ...
    'Private Sub CallOpt10059()
    '    Dim _clsCallOpt As New clsCallOpt

    '    _clsCallOpt.CallOpt10059()
    '    AddHandler _clsCallOpt.OnEventReturn10059ResultDt, AddressOf OnEventReturn10059ResultDt
    'End Sub

    'Private Sub OnEventReturn10059ResultDt(ByVal dt As DataTable)
...
#Region " Opt10059 "
    ...
    Private Sub OnReceiveTrData_opt10059(ByVal ds As DataSet)
        ...
        With _dt10059
            If _dt10059.Columns.Count < 1 Then
...
#Region " Opt10014 "
    ...
    Private Sub GetOpt10014()
        ...
        CallOpt10014(_stockCode, fromDate, toDate)

    End Sub

    Private Sub OnReceiveTrData_opt10014(ByVal ds As DataSet)
        If ds Is Nothing = True Then
            ComBineOpt10059WithOpt10014()
            Exit Sub
        End If
...
#Region " ComBineOpt10059WithOpt10081 "
    ...
    Private Sub ComBineOpt10059WithOpt10014()

        For column As Integer = _dtOpt10014.Columns.Count - 1 To 0 Step -1
            If _dtOpt10014.Columns(column).ColumnName = "일자" Then
                _dtOpt10014.Columns(column).ColumnName = "공매도일자"
            End If

            If _dtOpt10014.Columns(column).ColumnName = "종가" Then
                _dtOpt10014.Columns.RemoveAt(column)
            End If

            If _dtOpt10014.Columns(column).ColumnName = "전일대비기호" Then
                _dtOpt10014.Columns.RemoveAt(column)
            End If

            If _dtOpt10014.Columns(column).ColumnName = "전일대비" Then
                _dtOpt10014.Columns.RemoveAt(column)
            End If

            If _dtOpt10014.Columns(column).ColumnName = "등락율" Then
                _dtOpt10014.Columns.RemoveAt(column)
            End If

            If _dtOpt10014.Columns(column).ColumnName = "거래량" Then
                _dtOpt10014.Columns.RemoveAt(column)
            End If
        ...
        If _firstCall = True Then
            If clsFunc.DataTableColumnCloneToDataSet(_dt10059, _dtOpt10014) = False Then
                MsgBox("_dt10059에 컬럼 추가가 실패했습니다.", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
        End If
        ...
        With _dt10059
            For Each dr In _dt10059.Rows
                ...
                For Each dr2th In _dtOpt10014.Rows
                    If Trim(dr("일자").ToString()) = Trim(dr2th("공매도일자").ToString()) Then
                        For i As Integer = 0 To _dtOpt10014.Columns.Count - 1
                            dr(_dtOpt10014.Columns(i).ColumnName) = dr2th(_dtOpt10014.Columns(i).ColumnName)
                        Next
...
#End Region
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        UcStockChartBase1.SetDt = _dt10059
    End Sub
End Class
