...
Public Class ucVolumeChartBase
    ...
    Private Sub chartBase_MouseWheel(sender As Object, e As MouseEventArgs) Handles chartBase.MouseWheel
        If e.Delta < 0 Then
            chartBase.ChartAreas(0).AxisX.ScaleView.ZoomReset()
            chartBase.ChartAreas(0).AxisY.ScaleView.ZoomReset()
        End If

        If e.Delta > 0 Then
            Dim xMin As Double = chartBase.ChartAreas(0).AxisX.ScaleView.ViewMinimum
            Dim xMax As Double = chartBase.ChartAreas(0).AxisX.ScaleView.ViewMaximum
            Dim yMin As Double = chartBase.ChartAreas(0).AxisY.ScaleView.ViewMinimum
            Dim yMax As Double = chartBase.ChartAreas(0).AxisY.ScaleView.ViewMaximum

            Dim posXStart As Double = chartBase.ChartAreas(0).AxisX.PixelPositionToValue(e.Location.X) - (xMax - xMin) / 4
            Dim posXFinish As Double = chartBase.ChartAreas(0).AxisX.PixelPositionToValue(e.Location.X) + (xMax - xMin) / 4
            Dim posYStart As Double = chartBase.ChartAreas(0).AxisY.PixelPositionToValue(e.Location.Y) - (yMax - yMin) / 4
            Dim posYFinish As Double = chartBase.ChartAreas(0).AxisY.PixelPositionToValue(e.Location.Y) + (yMax - yMin) / 4

            chartBase.ChartAreas(0).AxisX.ScaleView.Zoom(posXStart, posXFinish)
            chartBase.ChartAreas(0).AxisY.ScaleView.Zoom(posYStart, posYFinish)
        End If

    End Sub

    Private Sub cmsArea1_Click(sender As Object, e As EventArgs) Handles cmsArea1.Click
        If chartBase.ChartAreas(0).Visible = True Then
            chartBase.ChartAreas(0).Visible = False
        Else
            chartBase.ChartAreas(0).Visible = True
        End If
    End Sub

    Private Sub cmsArea2_Click(sender As Object, e As EventArgs) Handles cmsArea2.Click
        If chartBase.ChartAreas(1).Visible = True Then
            chartBase.ChartAreas(1).Visible = False
        Else
            chartBase.ChartAreas(1).Visible = True
        End If
    End Sub
End Class
