...
Public Class ucStockAvgMagipPrice
    ...
    Public _stdDate As String = ""
    Private _PropWriteStockList As DataSet
    Private _PropReadStockList As DataSet
    Private _mode As Integer = 0 '0:ALL 1:한개씩
#Region " Property "
    Public Property ProStdDate As String
        Get
            Return _stdDate
        End Get
        Set(value As String)
    ...
    Public WriteOnly Property PropWriteStockList10001 As DataSet
        Set(value As DataSet)
            _mode = 1
            _PropWriteStockList = value
            If Not value Is Nothing Then
    ...
    Public WriteOnly Property PropWriteStockList10081 As DataSet
        Set(value As DataSet)
            _mode = 1
            _PropWriteStockList = value

            If Not value Is Nothing Then
                GetOpt10081(_stdDate)
            End If

        End Set
    End Property

    Public WriteOnly Property PropWriteStockList10082 As DataSet
        Set(value As DataSet)
            _mode = 1
            _PropWriteStockList = value
            If Not value Is Nothing Then
                GetOpt10082(_stdDate)
            End If
        End Set
    End Property
    ...
    Public WriteOnly Property MainStock As PaikRichStock.Common.ucMainStockVer2
        Set(value As PaikRichStock.Common.ucMainStockVer2)
            ...
            AddHandler _MainStock.OnReceiveTrData_opt10082, AddressOf OnReceiveTrData_opt10082
...
#Region " Opt10001 , Opt10007"
    ...
    Private Sub OnReceiveTrData_opt10001(ByVal ds As DataSet)
        ...
        If ds.Tables(0).Rows.Count < 1 Then GoTo Skip
...
#Region " Opt10081"
    ...
    Private Sub GetOpt10081(ByVal stdDateValue As String)
        ...
        If _PropWriteStockList Is Nothing Then Exit Sub
...
#Region " Opt10082"
    Private _dtOpt10082 As New DataTable
    Private _StockCodeOpt10082 As String = ""

    Private Sub CallOpt10082(ByVal stockCode As String, ByVal stdDate As String)
        _StockCodeOpt10082 = stockCode
        _MainStock.Opt10082_OnReceiveTrData(stockCode, _MainStock.GetStockInfo(stockCode), stdDate)
    End Sub

    Private Sub GetOpt10082(ByVal stdDateValue As String)
        Dim dr As DataRow

        If _dtOpt10082 Is Nothing = False Then
            _dtOpt10082.Reset()
        End If
        ...
        If strStockCode = "" Then
            MsgBox("내역이 없습니다. ", MsgBoxStyle.Exclamation)
        Else
            CallOpt10082(clsFunc.FirstCallSequnceOpt(strStockCode, ";"), _stdDate)
        End If

    End Sub

    Private Sub OnReceiveTrData_opt10082(ByVal ds As DataSet)
        If ds Is Nothing = True Then Exit Sub
        If ds.Tables(0).Rows.Count < 1 Then GoTo Skip
        If _StockCodeOpt10082 = "" Then Exit Sub
        If _StockCodeOpt10082 <> Trim(ds.Tables(0).Rows(0).Item("종목코드").ToString()) Then Exit Sub
...
Skip:
        ...
        With _dtOpt10082
            If _dtOpt10082.Rows.Count < 1 Then
                _dtOpt10082.Reset()
                If clsFunc.DataTableColumnCloneToDataSet(_dtOpt10082, ds) = False Then
                    MsgBox("_dtOpt10002에 컬럼 추가가 실패했습니다.", MsgBoxStyle.Exclamation)
                    Exit Sub
                End If
            ...
            If stockCode = "STOP" Then
                ...
                Exit Sub
            Else
                CallOpt10082(stockCode, _stdDate)
            End If
        End With
    End Sub
#End Region
