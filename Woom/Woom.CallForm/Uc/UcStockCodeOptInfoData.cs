...
namespace Woom.CallForm.Uc
{
    public partial class UcStockCodeOptInfoData : UserControl
    {
        ...
        public delegate void OnSelectedStockCodeHandler(string stockCode);
        public event OnSelectedStockCodeHandler OnSelectedStockCode;     
        ...
        private void dgvStockInfo_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvStockInfo.Rows[e.RowIndex].Cells["STOCK_CODE"].Value.ToString().Trim() != "")
            {
                object Handler = OnSelectedStockCode;
                OnSelectedStockCode(dgvStockInfo.Rows[e.RowIndex].Cells["STOCK_CODE"].Value.ToString().Trim());
            }
        }
    }
}
