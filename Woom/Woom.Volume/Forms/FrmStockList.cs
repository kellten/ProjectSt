    ...
    public partial class FrmStockList : Form
    {
        ...
        private void DisplayVolumeList(DataTable RecDt)
        {
            ...
            foreach (DataRow dr in RecDt.Rows)
            {
                ...
                dt = oKiwoomQuery.p_OPT10060PriceQuery(query: "2", stockCode: dr["STOCK_CODE"].ToString().Trim(), fromdate: dtpFromDate.Value.ToString("yyyyMMdd") ,toDate: dtpToDate.Value.ToString("yyyyMMdd"), maemeGb: "", bln3tier: false).Tables[0];
        ...
        private void DisplayVolumeListGigan(DataTable RecDt)
        {
            ...
            DataTable dt = new DataTable();
            int row = 0;
            string fromDate =   _clsUtilFunc.DateAddYear(_clsUtilFunc.DateToString(dtpFromDate.Value.ToString()), -3);
            string toDate = _clsUtilFunc.DateToString(dtpFromDate.Value.ToString());
            ...
            foreach (DataRow dr in RecDt.Rows)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    ViewGiganBySu(row, dr["STOCK_CODE"].ToString(), dt, _clsUtilFunc.DateToString(dtpFromDate.Text).ToString(), _clsUtilFunc.DateToString(dtpFromDate.Text).ToString(), "Today");
