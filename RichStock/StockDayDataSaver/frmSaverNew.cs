...
namespace StockDayDataSaver
{
    public partial class frmSaverNew : Form
    {
        ...
        private string _FILTERSTR = "STOCK_CODE <> '' AND STOCK_CODE NOT LIKE '%K' AND STOCK_NAME NOT LIKE '%ETN%' AND STOCK_NAME NOT LIKE '%스팩%' AND STOCK_NAME NOT LIKE '%선물%'  AND STOCK_NAME NOT LIKE '트러스%'  AND STOCK_NAME NOT LIKE 'KBSTAR%' AND STOCK_NAME NOT LIKE '하이골드%' AND STOCK_NAME NOT LIKE 'KINDEX%'  AND STOCK_NAME NOT LIKE 'KOSEF%'   AND STOCK_NAME NOT LIKE '하나니켈%'   AND STOCK_NAME NOT LIKE 'TREX%' AND STOCK_NAME NOT LIKE '코리아0%' AND STOCK_NAME NOT LIKE '동북아%'  AND STOCK_NAME NOT LIKE '아시아 %' AND STOCK_NAME NOT LIKE '%우B' AND STOCK_NAME NOT LIKE '%1호' AND STOCK_NAME NOT LIKE '%2호' AND STOCK_NAME NOT LIKE '%K' AND STOCK_NAME NOT LIKE '%채권%' AND STOCK_NAME NOT LIKE '%신탁%' AND STOCK_NAME NOT LIKE '%TIGER%' AND STOCK_NAME NOT LIKE '%KODEX%' AND STOCK_NAME NOT LIKE '%KINDEX%' AND STOCK_NAME NOT LIKE '%ARIRANG%' AND STOCK_NAME NOT LIKE '%(합성%'";
        ...
        public async void ExecuteEvening(int procNum = 0)
        {
            ...
            //dv.RowFilter = "STOCK_CODE >= '039230' AND " + _FILTERSTR;
            dv.Sort = "STOCK_CODE ASC";
            dataGridView1.DataSource = dv;
            ...
            if (procNum.Equals(0) || procNum.Equals(1)) { 
                _rowCnt = 0;
                foreach (DataRowView dr in dv)
                {
                    _arrsDelete.Clear();
                    _arrs.Clear();
                    tslStatus.Text = dr["STOCK_CODE"].ToString() + "-" + _rowCnt.ToString() + "- 일봉";
                    ...
                    System.Threading.Thread.Sleep(Cls.ValInt(txtSleep.Text));
                    _rowCnt++;
                }
            }

            if (procNum.Equals(0) || procNum.Equals(2))
            {
                _rowCnt = 0;
                foreach (DataRowView dr in dv)
                {
                    _arrsDelete.Clear();
                    _arrs.Clear();
                    tslStatus.Text = dr["STOCK_CODE"].ToString() + "-" + _rowCnt.ToString() + "- 매동";
            ...
            if (procNum.Equals(0) || procNum.Equals(3)) { 
                _rowCnt = 0;
                foreach (DataRowView dr in dv)
                {
                    _arrsDelete.Clear();
                    _arrs.Clear();
                    tslStatus.Text = dr["STOCK_CODE"].ToString() + "-" + _rowCnt.ToString() + "- 기본정보";
            ...
            if (procNum.Equals(0) || procNum.Equals(4))
            {
                //_rowCnt = 0;
                //foreach (DataRowView dr in dv)
                //{
                //    _arrsDelete.Clear();
                //    _arrs.Clear();
                //    tslStatus.Text = dr["STOCK_CODE"].ToString() + "-" + _rowCnt.ToString() + "- 주봉";
            ...
            if (procNum.Equals(0) || procNum.Equals(5)) { 
                //_rowCnt = 0;
                //foreach (DataRowView dr in dv)
                //{
                //    _arrsDelete.Clear();
        ...
        public void ExecuteMorning()
        {
            ...
            //    _arrsDelete.Clear();
            //    _arrs.Clear();
            //    tslStatus.Text = dr["STOCK_CODE"].ToString() + "-" + _rowCnt.ToString() + "-gong";
            //    await DoOpt10014Async(dr["STOCK_CODE"].ToString(), dr["STOCK_NAME"].ToString());
            //    if (_arrsDelete.Count > 0)
            //    {
            //        conn.ExecuteNonMultiDelete("stock_gong", _arrsDelete);
            ...
            //    _arrsDelete.Clear();
            //    _arrs.Clear();
            //    tslStatus.Text = dr["STOCK_CODE"].ToString() + "-" + _rowCnt.ToString() + "-credit";
        ...
        async Task DoOpt10013Async(string stockCode, string stockName)
        {
            ...
            else
            {
                ...
                ucMainStockVer2.Opt10013_OnReceiveTrDataNew(stockCode, stockName, dtpStdDate.Value.ToString("yyyyMMdd"), "1");
                var t1 = tcs.Task;
                if (await Task.WhenAny(t1, Task.Delay(3000)) == t1)
                {
                }
                else
                {
        ...
        {
            if (_mode == "evening") ExecuteEvening(0);
            else if (_mode == "evening1") ExecuteEvening(1);
            else if (_mode == "evening2") ExecuteEvening(2);
            else if (_mode == "evening3") ExecuteEvening(3);
