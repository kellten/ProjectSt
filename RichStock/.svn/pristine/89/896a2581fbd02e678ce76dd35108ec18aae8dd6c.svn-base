...
namespace NewsFinder
{
    public partial class frmMainVer2 : Form
    {
            ...
            foreach (DataRowView dr in dv)
            {
                if (chk자동매매.Checked == true)
                {
                    ...
                    if (chk공.Checked == true)
                    {
                        ...
                        SetDsScreenNo("A", "1", "1", dr["STOCK_CODE"].ToString().Trim(), dr["STOCK_NAME"].ToString().Trim(), true, "공");
                ...
                foreach (System.Xml.XmlNode node in forecastNodes)
                {
                    ...
                    if (!blnTrue)
                    {
                        ...
                        if (!dgr.Cells[T종목.Index].Value.Equals(""))
                        {
                            ...
                            NewsFavAdd(stockCode, "", "", "뉴", "", 0, 0, 0, 0);
                        }
        ...
        private void StoreJango()
        {
            ...
            arrs.Clear();
            arrsDelete.Clear();
            foreach (DataGridViewRow dgr in dg뉴스잔고.Rows)
            {
                if (dgr.Cells[J종목코드.Index].Value.Equals("")) continue;
                arr.Clear();
                arr.Add("ACCOUNT_NO", cboAccount.Text.Trim());
                arr.Add("STOCK_CODE", dgr.Cells[J종목코드.Index].Value.ToString().Trim());
                arrsDelete.Add(arr);

                arr.Clear();
                arr.Add("ACCOUNT_NO", cboAccount.Text.Trim());
                arr.Add("STOCK_CODE", dgr.Cells[J종목코드.Index].Value.ToString().Trim());
                arr.Add("SELL1_PRICE", dgr.Cells[J이익실현1.Index].Value.ToString().Trim());
                arr.Add("SELL2_PRICE", dgr.Cells[J이익실현2.Index].Value.ToString().Trim());
                arr.Add("SELL3_PRICE", dgr.Cells[J이익실현3.Index].Value.ToString().Trim());
                arr.Add("SELL4_PRICE", "0");
                arr.Add("SELL5_PRICE", "0");
                arr.Add("PLUS_PRICE", Cls.ValInt(dgr.Cells[J익절지정가.Index].Value));
                arr.Add("MINUS_PRICE", Cls.ValInt(dgr.Cells[J손절지정가.Index].Value));
                //if (dgr.Cells[J이익실현1.Index].Value.Equals("1")) 
                //    arr.Add("DEUNG_DATE1", DateTime.Now.ToString("yyyy-MM-dd 00:00:00"));
                //if (dgr.Cells[J이익실현2.Index].Value.Equals("1"))
                //    arr.Add("DEUNG_DATE2", DateTime.Now.ToString("yyyy-MM-dd 00:00:00"));
                //if (dgr.Cells[J이익실현3.Index].Value.Equals("1"))
                //    arr.Add("DEUNG_DATE3", DateTime.Now.ToString("yyyy-MM-dd 00:00:00"));
                arrs.Add(arr);
            }
            conn.ExecuteNonMultiDelete("stock_profit", arrsDelete);
            conn.ExecuteNonMultiInsert("stock_profit", arrs);
        }
            ...
            if (_ht최고체결강도[종목코드] != null)
            {
                ...
                else
                {
                    ...
                    //{
                        ...
                        if (isSell)
                        {
                            if (Cls.ValInt(dvr.Cells[J이익실현1.Index].Value) < 1)
                                이익실현차수 = 1;
                            else if (Cls.ValInt(dvr.Cells[J이익실현2.Index].Value) < 1)
                                이익실현차수 = 2;
                            else if (Cls.ValInt(dvr.Cells[J이익실현3.Index].Value) < 1)
                                이익실현차수 = 3;
                            else
                                이익실현차수 = 4;

                            int 익절지정가 = Cls.ValInt(dvr.Cells[J익절지정가.Index].Value);
                            int 손절지정가 = Cls.ValInt(dvr.Cells[J손절지정가.Index].Value);
        ...
        private void ProcessStopLoss(Double 수익률, DataSet ds, string 종목코드, string 종목명, int 매매가능수량, int 이익실현차수, DataGridViewRow dgrJan , int 익절지정가 , int 손절지정가)
        {
            ...
            DataRow drReal = ds.Tables[0].Rows[0];
            int 현재가 = Math.Abs(Cls.ValInt(drReal["현재가"]));
            int 최우선매도호가 = Math.Abs(Cls.ValInt(drReal["(최우선)매도호가"]));
            int 최우선매수호가 = Math.Abs(Cls.ValInt(drReal["(최우선)매수호가"]));

            var obj = (from DataGridViewRow dgr in dg뉴스체결.Rows
                       where dgr.Cells[N종목코드.Index].Value != null && dgr.Cells[N종목코드.Index].Value.Equals(종목코드) && (dgr.Cells[N모니터링구분.Index].Value.Equals("공") || dgr.Cells[N모니터링구분.Index].Value.Equals("뉴"))
                       select dgr).FirstOrDefault();
            ...
            if (is익절)
            {
                if (!익절지정가.Equals(0) && 현재가 < 익절지정가) return;
            ...
            if (is손절)
            {
                if (!손절지정가.Equals(0) &&  현재가 > 손절지정가) return;
        ...
        private void SettingTick(DataSet ds, DataTable dt)
        {
            ...
            int tradingprice = Math.Abs(price * volume);
            ...
            else
            {
                        ...
                        if(drR["매수신호"].Equals(1)) {
                            ...
                            if (obj != null && objMi != null)
                            {
                                try { 
                                    ...
                                    string tradingPrice = obj.Cells[N대금.Index].Value.ToString();
                                    CancelAll(obj.Cells[N종목코드.Index].Value.ToString());
                                    dg뉴스체결.Rows.Remove(obj);
                                    NewsFavAdd(dt.TableName, "Y", monPrice, monGb, tradingPrice, 0, 0, 0, 0);
                                }
                                catch { }
                            }
                        }
                ...
                else
                {
                    ...
                    if (!isBuy)
                    {
                        dr["매도거래량"] = Math.Abs((Math.Abs(Cls.ValInt(dr["매도거래량"])) + volume)) * -1;
                        dr["매도거래비용"] = Math.Abs((Math.Abs(Cls.ValInt(dr["매도거래비용"])) + tradingprice)) * -1;
                    }
                    else
                    {
                    ...
                    {
                        if (Math.Abs(Cls.ValInt(dr["매수거래비용"])) - Math.Abs(Cls.ValInt(dr["매도거래비용"])) < Math.Abs(Cls.ValInt(nm조건거래대금.Value)) * 1000000 / 3)
                        {
                            dr["매수신호"] = 0;
                        }
                        else if (Math.Abs(Cls.ValInt(dr["매수거래비용"])) - Math.Abs(Cls.ValInt(dr["매도거래비용"])) < Cls.ValInt(nm조건거래대금.Value) * 1000000 * -1)
                        {
                            dr["매도신호"] = 1;
                        }
                    } 
        ...
        private void SettingOPT10007(DataSet ds)
        {
            ...
            if (obj != null)
            {
                ...
                dgr.Cells[J등락율.Index].Value = 등락률.ToString("0.00");
                dgr.Cells[J대비.Index].Value = 대비;
            }

            var objN = (from DataGridViewRow dgr in dgN관종.Rows
            ...
            {
                var dgrN = objN;
                dgrN.Cells[P현재가.Index].Value = 현재가;
                dgrN.Cells[P등락률.Index].Value = 등락률.ToString("0.00");
            }
            //TAB 0 처리 - E
        ...
        private DataTable SetLine(string 종목코드)
        {
            ...
            DataRow dr;
            DataRow[] drArr;
            if (_ds전체일봉.Tables.Count < 1) return null;
        ...
        private DataTable SetBuySellState(string 종목코드)
        {
            ...
            if (_ds전체매동.Tables.Count < 1) return null;
        ...
        private DataTable SetFinance(string 종목코드)
        {
            ...
            if (_ds전체재정.Tables.Count < 1) return null;
        ...
        private void SettingNewsFav(DataSet ds)
        {
            ...
            if (Cls.Val(저가) + ((Cls.Val(고가) - Cls.Val(저가)) * 매수신호기준비율) <= Cls.Val(현재가))
            {
                if (!모니터링구분.Equals("수"))
                {
                    if (diffSecond > 60)
                    {
                        if (모니터링구분.Equals("공") || 모니터링구분.Equals("수"))
                        {
                            SetDsScreenNo("D", "1", "1", 종목코드, 종목명, false,"공");
                        }
            ...
            if (모니터링구분.Equals("공") || 모니터링구분.Equals("뉴"))
            {
                if (diffSecond > 조건시간)
                {
        ...
        skip :
            if (isBuy)
            {
                ...
                else if (obj.Cells[P모니터링구분.Index].Value.Equals("공") || obj.Cells[P모니터링구분.Index].Value.Equals("뉴")) 
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (Cls.isExistsRow(dg뉴스체결, 0, 종목코드) == -1)
                        {
                ...
                objOrder.Cells[N대금.Index].Value = obj.Cells[P대금.Index].Value;
    ...
    skip :
            if (isNagari)
            {
                ...
                string tradingPrice = dgr.Cells[N대금.Index].Value.ToString();
                CancelAll(종목코드);
                dg뉴스체결.Rows.Remove(dgr);

                if (!monGb.Equals("공")) { 
                    ResetSignal(종목코드, 2);
                    NewsFavAdd(종목코드, "Y", monprice, monGb, tradingPrice , 0, 0, 0, 0, false);
                }
            }
            //체결쪽으로 넘어간 데이터를 검수하고 RealData를 업데이트 해준다. - E
        }
        ...
        private async Task DoOpt10007(string stockCode, string stockName)
        {
            ...
            {
                Console.WriteLine("시세표 Load Delay " + stockName);
            }
        }
        ...
        private async void SetDsScreenNo(string actGb, string 화면구분, string 실시간구분, string 종목코드, string 종목명, bool isOPT10007 , string monGb)
        {
            DataRow drAdd;
            DataRow[] drArr;
                ...
                if (실시간구분 == "1")
                {
                    ...
                    if (종목코드.Length == 6)
                    {
                        if (_dsTick60.Tables[종목코드] == null)
                        {
                            ...
                            if (!monGb.Equals("공") && !monGb.Equals("뉴")) { 
                                SetTickImport(종목코드);
                            }
                        }
                    }
                    else
                    {
                        foreach (string str in 종목코드.Split(';'))
                        {
                            ...
                            if (_dsTick60.Tables[str] == null)
                            {
                                ...
                                {
                                    cboTickDataMember.Items.Add(str + "-" + ucMainStockVer2.GetStockInfo(str));
        ...
        private void SetConditionList(DataSet ds, string index, string scrNo, string conName)
        {
            ...
            try
            {
                ...
                {
                    SetDsScreenNo("A", "3", "1", strCodes, "", false ,"조");
                    SystemSleep();
                }
                else
                {
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            for (int row = 0; row < dgN관종.RowCount - 1; row++)
            {
                SetDsScreenNo("D", "1", "1", dgN관종.Rows[row].Cells[P종목코드.Index].Value.ToString().Trim(), dgN관종.Rows[row].Cells["P종목명"].Value.ToString().Trim(), false, "공");
            }
            //_ds뉴스.Tables.Clear();
            dgN관종.RowCount = 0;
        }
        ...
        private void btn뉴스체결_Click(object sender, EventArgs e)
        {
            ...
            {
                SetDsScreenNo("D", "1", "1", dg뉴스체결.Rows[row].Cells[N종목코드.Index].Value.ToString().Trim(), dg뉴스체결.Rows[row].Cells["N종목명"].Value.ToString().Trim(), false,"공");
            }
            dg뉴스체결.RowCount = 0;
        }
        //체결된 실시간 리스트 리셋 - E
        ...
        private void dgN관종_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex < 1)
            {
                SetDsScreenNo("A", "1", "1", dgN관종.Rows[e.RowIndex].Cells[P종목코드.Index].ToString().Trim(), dgN관종.Rows[e.RowIndex].Cells["P종목명"].ToString().Trim(), true,"수");
            }
        }
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (blnTrue == false)
                {
                    try
                    {
                        ...
                        SetDsScreenNo("A", "4", "1", dv[0]["STOCK_CODE"].ToString().Trim(), dv[0]["STOCK_NAME"].ToString().Trim(), true,"관");
                    }
                    finally
                    {
                    }
        ...
        private void dg관종_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0) //삭제버튼클릭시
            {
                ...
                if (dg관종.Rows[e.RowIndex].Cells[F_종목코드.Index].Value != null) {
                    SetDsScreenNo("D", "4", "1", dg관종.Rows[e.RowIndex].Cells[F_종목코드.Index].Value.ToString().Trim(), dg관종.Rows[e.RowIndex].Cells["F_종목명"].Value.ToString().Trim(), false,"관");
                }
        ...
        private void GetStockDayDataQuery()
        {
            ...
            if (!cboAccount.Text.Equals("5116998410") || Cls.ValInt(DateTime.Now.ToString("HH")) >= 19)
            {
                arrParam.Clear();
        ...
        private void SetTickInit()
        {
            ...
            if (_dsTickInit.Tables[0].Rows.Count > 0)
            {
                ...
                SetDsScreenNo("A", "4", "1", stockCodes, "", false , "공");
                ...
                foreach (DataRowView drTick in dv)
                {
                    ...
                    if (_dsTick60.Tables[drTick["STOCK_CODE"].ToString().Trim()].Rows.Count >= 50) continue;
        ...
        private void SetTickImport(string stockCode)
        {
            DataRow tickRow;
            _dsTickInit = _DataAcc.p_stock_tick_query("2", _stockId, "", stockCode, "", null, null);
            if (_dsTickInit.Tables[0].Rows.Count > 0)
            {
                DataView dv = new DataView(_dsTickInit.Tables[0]);
                dv.Sort = "STOCK_DATE DESC , S_TIME DESC";
        ...
        private void GetFavList(string InterId)
        {
            ...
            try
            {
                ...
                {
                    SetDsScreenNo("A", "4", "1", stockCodes, "", true,"관");
                }
            }
            finally
            {
        ...
        private Task SettingJango(DataSet ds)
        {
            ...
            var objNewsOrder = (from DataGridViewRow dgr in dg뉴스체결.Rows
                                ...
                                where dgr.Cells[N종목코드.Index].Value != null && dgr.Cells[N종목코드.Index].Value.Equals(stockCode) && (dgr.Cells[N모니터링구분.Index].Value.Equals("공"))
                               select dgr).FirstOrDefault();
            ...
            else //체결
            {
                if (objJangoRow == null && dr["주문구분"].ToString().IndexOf("매수") > -1) //잔고에 없을시에 새로 만들어준다
                {
                    ...
                    SetDsScreenNo("A", "2", "1", stockCode, "", false,"잔");
                }
            ...
            else 
            {
                if (구분 == "I")
                {
                    ...
                    try
                    {
                        ...
                        {
                            SetDsScreenNo("A", "3", "1", 종목코드, dr["STOCK_NAME"].ToString().Trim(), false,"조");
                        }
        ...
        private void UcMainStockVer2_OnReceiveTrData_Opw00018(DataSet ds)
        {
            ...
            for (int row = 0; row < ds.Tables[0].Rows.Count ; row++)
            {
                ...
                if (_ds익절.Tables.Count < 1 || _ds익절.Tables[0].Select(String.Format("STOCK_CODE = '{0}'", dgr.Cells[J종목코드.Index].Value)).Length == 0)
                {
                ...
                else
                {
                    foreach (DataRow dr in _ds익절.Tables[0].Select(String.Format("STOCK_CODE = '{0}'", dgr.Cells[J종목코드.Index].Value)))
                    {
                        ...
                        dgr.Cells[J익절지정가.Index].Value = dr["PLUS_PRICE"];
                        dgr.Cells[J손절지정가.Index].Value = dr["MINUS_PRICE"];
                        break;
                    }
                }
                dg뉴스잔고.CellValueChanged += dg뉴스잔고_CellValueChanged;
            ...
            SetDsScreenNo("A", "2", "1", strStockCodes, "", true,"잔");
            SystemSleep();
        }
        ...
        private void dg관종_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == F_거래량.Index)
            {
                ...
                if (dg관종.Rows[e.RowIndex].Cells[F_모니터링.Index].Value.Equals("하한가") && 등락률 <= -29.7)
                {
                    if (저가.Equals(현재가)) return;
                    int 수량 = Cls.ValInt(nm관종매수금액.Value) / 저가;

                    if (!저가.Equals(현재가))
                    {
                        if (_dsTick60.Tables[stockCode].Select("일자 = '" + DateTime.Now.ToString("yyyyMMdd") + "' AND 매수유무 = '1'").Length < 1)
                        {
                            SendBuySellMsg(stockCode, "00", 1, 0, 저가, 수량, "2");
                        }
                    }
                }
            }
        ...
        private void 시장가매수ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else if (dgObj.Name == dg뉴스체결.Name) { stockCode = dgr.Cells[N종목코드.Index].Value.ToString().Trim(); 현재가 = Cls.ValInt(dgr.Cells[N현재가.Index].Value); }
        ...
        private void NewsFavAdd(string stockCode , string isForce , string monPrice , string monGb , string tradingPrice , int 시가 , int 고가 , int 저가 , int 현재가 , bool chkCnt = true)
        {
            
            string stockName = ucMainStockVer2.GetStockInfo(stockCode);
            if (!monGb.Equals("공") && !monGb.Equals("뉴") && !monGb.Equals("수"))
            {
            ...
            if (obj == null && objOrder == null)
            {
                ...
                if (chk자동매매.Checked)
                {
                    ...
                    if (!monGb.Equals("공") && !monGb.Equals("뉴"))
                    {
                        DataRow[] drArr = _ds전체재정.Tables[0].Select(String.Format("STOCK_CODE = '{0}'", stockCode));
                        if (drArr.Length > 0)
                        {
                            DataRow dr = drArr[0];
                            if (Cls.Val(dr["VOLUME"]) <= 200000) tradingPrice = "10";
                            else if (Cls.Val(dr["VOLUME"]) <= 300000) tradingPrice = "12";
                            else if (Cls.Val(dr["VOLUME"]) <= 400000) tradingPrice = "15";
                            else if (Cls.Val(dr["VOLUME"]) <= 500000) tradingPrice = "18";
                            else tradingPrice = "21";

                            if (Math.Abs(Cls.ValInt(dr["END_PRICE"])) >= 50000) tradingPrice = "50";
                        }
                    }
                    dgr.Cells[P모니터링가격.Index].Value = monPrice;                    
                    dgr.Cells[P모니터링구분.Index].Value = monGb;
                    dgr.Cells[P대금.Index].Value = tradingPrice;

                    SetDsScreenNo("A", "1", "1", stockCode, stockName, false, monGb);
                    SetTickImport(stockCode);
                }
                //뉴스 관종에 등록 - E
            }
        ...
        private void dgN관종_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Control == true && e.KeyCode == Keys.D)
            {
                ...
                {
                    SetDsScreenNo("D", "1", "1", stockCode, stockName, false,"공");
                }
        ...
        private void dg뉴스잔고_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (e.ColumnIndex == J보유수량.Index)
            {
                ...
                if (Cls.Val(dgr.Cells[J보유수량.Index].Value.ToString()) <= 0) {
                    ...
                    SetDsScreenNo("D", "2", "1", dgr.Cells[J종목코드.Index].Value.ToString(), dgr.Cells[J종목명.Index].Value.ToString(), false,"잔");
        ...
        //    DataRow[] drs;
        //    DataRow drObj;
        //    if (_ds익절.Tables.Count < 1) return;
        ...
        private void 차트ToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            ...
            var objJ = (from DataGridViewRow dgr in dgN관종.Rows
                        where dgr.Cells[P종목코드.Index].Value != null && dgr.Cells[P종목코드.Index].Value.Equals(stockCode)
                        select dgr).FirstOrDefault();

            if (objJ != null)
            {
                initPrice = Cls.ValInt(objJ.Cells[P모니터링가격.Index].Value);
            }

            if (initPrice.Equals(0))
            {
                var objO = (from DataGridViewRow dgr in dg뉴스체결.Rows
                            where dgr.Cells[N종목코드.Index].Value != null && dgr.Cells[N종목코드.Index].Value.Equals(stockCode)
                            select dgr).FirstOrDefault();

                if (objO != null)
                {
                    initPrice = Cls.ValInt(objO.Cells[N모니터링가격.Index].Value);
                }
            }

            if (stockCode == "") return;

            Chart.frmChart frmChart = new Chart.frmChart(initPrice);
        ...
        private void GetThemeList(string themeName)
        {
            ...
            try
            {
                ...
                //    )
                if (Cls.IsDb(_ds전체일봉) == true) { }
                else{SetDsScreenNo("A", "4", "1", stockCodes, "", true,"관");}
        ...
        private void GetJangoList()
        {
            ...
            try
            {
                ...
                if (Cls.IsDb(_ds전체일봉) != true)
                {
                    SetDsScreenNo("A", "4", "1", stockCodes, "", true,"잔");
                }
            }
            finally
            {
        ...
        private void dgN관종_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Cls.Val(dgr.Cells[P현재가.Index].Value) == 0) return;
            ...
            if (Cls.Val(dgr.Cells[P현재가.Index].Value) <= Cls.Val(dgr.Cells[P모니터링가격.Index].Value) * (1 + Cls.Val(nm이격.Value) * 0.01))
            {
                dgr.DefaultCellStyle.BackColor = System.Drawing.Color.MistyRose;
            }
            else
            {
                dgr.DefaultCellStyle.BackColor = System.Drawing.Color.Empty;
            }
        }
        ...
        private void NewsOrderAdd(string stockCode, string isForce, string monPrice, string monGb, string tradingPrice , int 시가, int 고가, int 저가, int 현재가)
        {
            string stockName = ucMainStockVer2.GetStockInfo(stockCode);
            var objOrder = (from DataGridViewRow dgr in dg뉴스체결.Rows
                            where dgr.Cells[N종목코드.Index].Value != null && dgr.Cells[N종목코드.Index].Value.Equals(stockCode)
            ...
            if (objOrder == null)
            {
                ...
                dgr.Cells[N대금.Index].Value = tradingPrice;
                //뉴스 체결에 등록 - E
            }
        }
        ...
        private void 되돌리기ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dgObj.Name == dg뉴스체결.Name) stockCode = dgObj.Rows[row].Cells[N종목코드.Index].Value.ToString();
            else return;

            if (stockCode == "") return;

            string monPrice = dgObj.Rows[row].Cells[N모니터링가격.Index].Value.ToString();
            string monGb = dgObj.Rows[row].Cells[N모니터링구분.Index].Value.ToString();
            string tradingPrice = dgObj.Rows[row].Cells[N대금.Index].Value.ToString();
            
            dgObj.Rows.Remove(dgObj.Rows[row]);
            ResetSignal(stockCode, 2);
            NewsFavAdd(stockCode, "Y", monPrice, monGb, tradingPrice, 0, 0, 0, 0);
        }

        private void btnExcel_Click(object sender, EventArgs e)
        {

            foreach (DataGridViewRow dvr in dgN관종.Rows)
            {
                _DataAcc.p_Psi02Add("A", _stockId, 1, dvr.Cells[P종목코드.Index].Value.ToString(), "00", "", "", null, null); 
            }

            foreach (DataGridViewRow dvr in dg뉴스체결.Rows)
            {
                _DataAcc.p_Psi02Add("A", _stockId, 1, dvr.Cells[N종목코드.Index].Value.ToString(), "00", "", "", null, null);
            }

            DataGridView[] dgArr = { dgN관종, dg뉴스체결 };
            Export_ExcelColumn(dgArr, String.Format("C:\\{0}.xlsx", DateTime.Now.ToString("yyyyMMdd")));
            MessageBox.Show(String.Format("C:\\{0}.xlsx", DateTime.Now.ToString("yyyyMMdd")) + " 경로에 저장 완료 되었습니다.");
        }
        ...
        private void btnExcelImport_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog1.Filter = "엑셀파일 (*.xlsx)|*.xlsx";
            if (openFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.Cancel) return;

            Import_Excel(dgN관종, openFileDialog1.FileName);
        }

        private static void Export_Excel(DataGridView[] dGVArr, string filename)
        {
            ...
            int row = 0;
            for (int h = 0; h < dGVArr.Length; h++)
            {
                DataGridView dGV = dGVArr[h];

                for (int i = 1; i < dGV.Columns.Count + 1; i++)
                {
                    objexcelapp.Cells[row + 1, i] = dGV.Columns[i - 1].HeaderText;
                }
                ...
                for (int i = 0; i < dGV.Rows.Count; i++)
                {
                    for (int j = 0; j < dGV.Columns.Count; j++)
                    {
                        ...
                        {
                            objexcelapp.Cells[row + 1, j + 1] = dGV.Rows[i].Cells[j].Value.ToString();
                        }
                    }
                    row++;
                }
        ...
        private void Export_ExcelColumn(DataGridView[] dGVArr, string filename, params int[] cols)
        {
            Microsoft.Office.Interop.Excel.Application objexcelapp = new Microsoft.Office.Interop.Excel.Application();
            ...
            try
            {
                ...
                objexcelapp.Cells[1, 1] = "종목코드";
                objexcelapp.Cells[1, 2] = "종목명";
                objexcelapp.Cells[1, 3] = "모니터링가격";
                objexcelapp.Cells[1, 4] = "대금";
                objexcelapp.Cells[1, 5] = "모니터링구분";


                int row = 1;
                for (int i = 0; i < dGVArr[0].Rows.Count; i++)
                {
                    objexcelapp.Cells[row + 1, 1] = dGVArr[0].Rows[i].Cells[P종목코드.Index].Value.ToString(); //종목코드
                    objexcelapp.Cells[row + 1, 2] = dGVArr[0].Rows[i].Cells[P종목명.Index].Value.ToString(); //종목명
                    objexcelapp.Cells[row + 1, 3] = dGVArr[0].Rows[i].Cells[P모니터링가격.Index].Value == null ? "" : dGVArr[0].Rows[i].Cells[P모니터링가격.Index].Value.ToString(); //모니터링가격
                    objexcelapp.Cells[row + 1, 4] = dGVArr[0].Rows[i].Cells[P대금.Index].Value == null ? "" : dGVArr[0].Rows[i].Cells[P대금.Index].Value.ToString(); //대금
                    objexcelapp.Cells[row + 1, 5] = dGVArr[0].Rows[i].Cells[P모니터링구분.Index].Value == null ? "" : dGVArr[0].Rows[i].Cells[P모니터링구분.Index].Value.ToString(); //모니터링구분
                    row++;
                }


                for (int i = 0; i < dGVArr[1].Rows.Count; i++)
                {
                    objexcelapp.Cells[row + 1, 1] = dGVArr[1].Rows[i].Cells[N종목코드.Index].Value.ToString(); //종목코드
                    objexcelapp.Cells[row + 1, 2] = dGVArr[1].Rows[i].Cells[N종목명.Index].Value.ToString(); //종목명
                    objexcelapp.Cells[row + 1, 3] = dGVArr[1].Rows[i].Cells[N모니터링가격.Index].Value.ToString(); //모니터링가격
                    objexcelapp.Cells[row + 1, 4] = dGVArr[1].Rows[i].Cells[N대금.Index].Value.ToString(); //대금
                    objexcelapp.Cells[row + 1, 5] = dGVArr[1].Rows[i].Cells[N모니터링구분.Index].Value.ToString(); //모니터링구분
                    row++;
                }
        ...
        private void Import_Excel(DataGridView dgv, string fileName)
        {
            Microsoft.Office.Interop.Excel.Application objexcelapp = new Microsoft.Office.Interop.Excel.Application();
            Microsoft.Office.Interop.Excel._Workbook xlWorkbook = objexcelapp.Application.Workbooks.Open(fileName);
            ...
            try
            {
                ...
                {
                    if (xlRange.Cells[row, 1].Value == null || xlRange.Cells[row, 1].Value.Equals("")) continue;
                    dgv.RowCount++;
                    string stockCode = Cls.ValInt(xlRange.Cells[row, 1].Value).ToString("000000"); //종목코드
                    string stockName = xlRange.Cells[row, 2].Value.ToString(); //종목명
                    string monPrice = xlRange.Cells[row, 3].Value == null ? "" : xlRange.Cells[row, 3].Value.ToString(); //모니터링가격
                    string monTradingValue = xlRange.Cells[row, 4].Value == null ? "" : xlRange.Cells[row, 4].Value.ToString(); //대금
                    string monGb = xlRange.Cells[row, 5].Value == null ? "" : xlRange.Cells[row, 5].Value.ToString(); //대금

                    DataGridViewRow dgr = dgv.Rows[dgv.RowCount - 1];
                    dgr.Cells[P종목코드.Index].Value = stockCode;
                    dgr.Cells[P종목명.Index].Value = stockName;
                    dgr.Cells[P최초거래시간.Index].Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                    dgr.Cells[P강제추가여부.Index].Value = "Y";
                    dgr.Cells[P모니터링가격.Index].Value = monPrice;
                    dgr.Cells[P모니터링구분.Index].Value = monGb;
                    dgr.Cells[P대금.Index].Value = monTradingValue;

                    if (Cls.IsDb(_ds전체일봉))
                    {
                        DataRow[] drs = _ds전체일봉.Tables[0].Select(String.Format("STOCK_CODE = '{0}'", stockCode));
                        if (drs.Length > 0)
                        {
                            dgr.Cells[P현재가.Index].Value = drs[0]["END_PRICE"];
                        }
                        
                    }
                    
                    stockCodes += stockCode + ";";
                    //SetDsScreenNo("A", "2", "1", stockCode, stockName, false);
                }

                if (!Cls.IsDb(_ds전체일봉)) 
                    SetDsScreenNo("A", "1", "1", stockCodes, "", true, "수");
                else
                    SetDsScreenNo("A", "1", "1", stockCodes, "", false, "수");
            }
            finally
            {
                objexcelapp.Quit();
        ...
        private void dg뉴스잔고_KeyDown(object sender, KeyEventArgs e)
        {

            if (e.Control == true && e.KeyCode == Keys.A)
            {
                for (int row = 0; row < dg뉴스잔고.RowCount; row++)
                {
                    dg뉴스잔고.Rows[row].Visible = true;
                }
                return;
            }

            if (dg뉴스잔고.CurrentRow.Index == -1) return;
            if (dg뉴스잔고.Rows[dg뉴스잔고.CurrentRow.Index].Cells[J종목코드.Index].Value == null) return;
            if (e.Control == true && e.KeyCode == Keys.D)
            {
                dg뉴스잔고.Rows[dg뉴스잔고.CurrentRow.Index].Visible = false;
            }
        }

        private void txt뉴스관종검색_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                foreach (DataGridViewRow dgr in dgN관종.Rows)
                {
                    if (dgr.Cells[P종목명.Index].Value == null) break;
                    if (txt뉴스관종검색.Text.Equals(dgr.Cells[P종목명.Index].Value)) dgr.Visible = true;
                    else dgr.Visible = false;
                }
            }
        }

        private void txt뉴스관종검색_TextChanged(object sender, EventArgs e)
        {
            if (txt뉴스관종검색.Text.Equals(""))
            {
                foreach (DataGridViewRow dgr in dgN관종.Rows)
                {
                    if (dgr.Cells[P종목명.Index].Value == null) break;
        ...
        private void 주시장가매수ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string stockCode = "";
            int 매수수량 = 0;
            int 현재가 = 0;
            ...
            매수수량 = Cls.ValInt(nm관종매수금액.Value) / Cls.ValInt(현재가);
            SendBuySellMsg(stockCode, "03", 1, 0, 0, 1, "2");
        }
    }
}
